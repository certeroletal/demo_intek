...
Sub CreatePresentationFromMarkdown()
    ' Script para crear una presentación de PowerPoint desde un texto en formato Markdown.

    ' Contenido de la presentación en Markdown
...
"""
    ...
    Dim slidesContent() As String
    slidesContent = Split(mdContent, "---")

    Dim pres As Presentation
    Set pres = Presentations.Add

    Dim i As Integer
    For i = 0 To UBound(slidesContent)
        Dim slideText As String
        slideText = Trim(slidesContent(i))

        If Len(slideText) > 0 Then
            Dim sld As Slide
            Dim layout As CustomLayout

            ' Usar diseño de Título y Contenido, o solo título si no hay cuerpo
            If InStr(slideText, vbCrLf) > 0 Then
                Set layout = pres.SlideMaster.CustomLayouts(ppLayoutText)
            Else
                Set layout = pres.SlideMaster.CustomLayouts(ppLayoutTitleOnly)
            End If
            
            Set sld = pres.Slides.AddSlide(pres.Slides.Count + 1, layout)

            Dim lines() As String
            lines = Split(slideText, vbCrLf)

            Dim titleLine As String
            Dim bodyContent As String
            Dim isCodeBlock As Boolean
            isCodeBlock = False
            ...
            For j = 0 To UBound(lines)
                ...
                If InStr(currentLine, "###") = 1 Then
                    titleLine = Trim(Replace(currentLine, "###", ""))
                    ' Quitar negrita del título si existe
                    titleLine = Replace(titleLine, "**", "")
                    sld.Shapes.Title.TextFrame.TextRange.Text = titleLine
                ElseIf InStr(currentLine, "```") = 1 Then
                     isCodeBlock = Not isCodeBlock
                     If isCodeBlock Then bodyContent = bodyContent & vbCrLf ' Espacio antes del bloque de código
                ElseIf Len(currentLine) > 0 Then
                    ' Limpiar la línea de markdown
                    currentLine = Replace(currentLine, "*   ", "")
                    currentLine = Replace(currentLine, "**", "") ' Quitar negrita
                    
                    If isCodeBlock Then
                        bodyContent = bodyContent & currentLine & vbCrLf
                    Else
                        bodyContent = bodyContent & currentLine & vbCrLf
                    End If
                End If
            ...
            If Len(bodyContent) > 0 Then
                sld.Shapes(2).TextFrame.TextRange.Text = bodyContent
                If isCodeBlock Then
                    With sld.Shapes(2).TextFrame.TextRange.Font
                        .Name = "Courier New"
                        .Size = 10
                    End With
                End If
