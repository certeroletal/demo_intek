...
mode: agent
---
Usando como base las instrucciones `.github/copilot-instructions.md` en la sección "## Estructura de Directorios"
Crea un script (create_structure.sh) en la carpeta `03_scripts` que permita crear la estructura de carpetas y y la totalidad de los archivos 
Los subproyectos son los siguientes: ${input:subproyectos}
El script debe considerar la ruta base del proyecto como base para todo
Se debe otorgar permisos de ejecución  y ejecutar el script

El siguiente es un ejemplo de script que crea la estructura de carpetas y archivos para los siguientes subproyectos: adm, ops, web, ctrl

```bash
#!/bin/bash
# Script para crear la estructura de carpetas y archivos base siguiendo arquitectura hexagonal
# Subproyectos: adm, ops, web, ctrl
...
mkdir -p "$BASE_DIR/.devcontainer/adm" "$BASE_DIR/.devcontainer/ops" "$BASE_DIR/.devcontainer/web" "$BASE_DIR/.devcontainer/ctrl"
...
touch "$BASE_DIR/.devcontainer/ctrl/devcontainer.json"
...
# Subproyectos
for sub in adm ops web ctrl; do
  case $sub in
    adm) idx=4;;
    ops) idx=5;;
    web) idx=6;;
